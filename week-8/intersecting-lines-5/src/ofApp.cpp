...
void ofApp::setup(){
    for (int i = 0; i < 100; i++) {
        ...
        float randomAngle = (int)ofRandom(0, 10) * PI / 2; // ofRandom(0, TWO_PI);
        ofPoint b = a + 50 * ofPoint(cos(randomAngle), sin(randomAngle));
        lineSegment segment;
        segment.a = a;
        segment.b = b;
...
void ofApp::draw(){
    ...
    for (int y = 0; y < 100; y++) {
        ...
        curPos.set(512, 512);
        curDir = ofPoint(cos(ofGetElapsedTimef() + y * 0.01) * 2, sin(ofGetElapsedTimef() + y * 0.001) * 2);
        
        // for 10 times
//        ofSetHexColor(0x5BDB99);
        ofSetColor(ofColor::fromHsb(ofMap(atan2(curDir.y, curDir.x), -PI, PI, 0, 255), 180, 180));
        for (int z = 0; z < 10; z++) {
